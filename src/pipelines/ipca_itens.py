...
def filter_level_4(df):
    """Filtra somente ITENS (nível 4)."""
    df = df.copy()
    df["nivel"] = df["categoria"].apply(lambda x: len(str(x).split(".")[0]))
    return df[df["nivel"] == 4].copy()
...
def get_ipca_itens(periodo="all"):
    """Retorna o DataFrame final de IPCA itens (nível 4)."""

    df_valor, df_peso = load_ipca_raw(periodo)

    df_valor = filter_level_4(df_valor)
    df_peso = filter_level_4(df_peso)
...
def run_ipca_itens(periodo="all"):
    """
    Executa o pipeline do IPCA itens:
    - consulta o SIDRA (tabela 7060)
    - monta o DataFrame com Indice, Peso e categorias
    - cria/atualiza a tabela bcb.ipca_itens
    """
    df = get_ipca_itens(periodo)

    ensure_table(df, "ipca_itens", schema="bcb")
    insert_all(df, "ipca_itens", schema="bcb")

    print("Tabela bcb.ipca_itens atualizada com sucesso!")
