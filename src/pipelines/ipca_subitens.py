...
def load_ipca_raw(periodo="all"):
    ...
    df_valor = sidra_query(
        tabela=7060, variavel=63, periodo=periodo, classificacao={"315": "all"}
    ).rename(columns={"valor": "indice"})

    df_peso = sidra_query(
        tabela=7060, variavel=66, periodo=periodo, classificacao={"315": "all"}
    ).rename(columns={"valor": "peso"})

    return df_valor.copy(), df_peso.copy()


def filter_level_7(df):
    """Filtra somente subitens (nível 7)."""
    df = df.copy()
    df["nivel"] = df["categoria"].apply(lambda x: len(str(x).split(".")[0]))
    return df[df["nivel"] == 7].copy()
...
def split_category(df):
    ...
    df["category_name"] = df["categoria"].apply(lambda x: str(x).split(".")[1])
    return df
...
# ============================================================
# 2. Função principal de dados (equivalente a get_ipca_geral)
# ============================================================


def get_ipca_subitens(periodo="all"):
    """Retorna o DataFrame final de IPCA subitens (nível 7)."""

    df_valor, df_peso = load_ipca_raw(periodo)

    df_valor = filter_level_7(df_valor)
    df_peso = filter_level_7(df_peso)
...
# ============================================================
# 3. Função pipeline (equivalente a run_ipca)
# ============================================================


def run_ipca_subitens(periodo="all"):
    """
    Executa o pipeline do IPCA subitens:
    - consulta o SIDRA (tabela 7060)
    - monta o DataFrame com Indice, Peso e categorias
    - cria/atualiza a tabela bcb.ipca_subitens
    """
    df = get_ipca_subitens(periodo)

    ensure_table(df, "ipca_subitens", schema="bcb")
    insert_all(df, "ipca_subitens", schema="bcb")

    print("Tabela bcb.ipca_subitens atualizada com sucesso!")
